...
├── lib/
│   ├── battery_info_example.dart              # Platform Channel API
│   ├── battery_info_jni_wrapper.dart          # JNI wrapper
│   └── src/generated/                         # jnigen output
│       └── battery_info_jni.dart
├── android/
│   └── src/main/kotlin/.../                   # Kotlin code
│       ├── BatteryInfoExamplePlugin.kt        # Platform Channel handler
│       └── BatteryInfoJni.kt                  # JNI helper class
├── example/
│   └── lib/main.dart                          # Benchmark UI
├── jnigen.yaml                                # jnigen configuration
└── pubspec.yaml
```

## 🔬 Deep Dive: How Each Approach Works
...
- **Method Channel**: 200-400μs (includes serialization + async messaging)
- **JNI**: 20-80μs (direct native call)
...
✅ Building standard Flutter plugins
✅ Performance is not critical
✅ You want simple, maintainable code
✅ You're new to Flutter plugin development
✅ You need async operations (channels are inherently async)

### Use JNI (with jnigen) When:
✅ You have existing Java/Kotlin libraries to wrap
✅ Performance is critical (e.g., real-time processing)
✅ You need to call native code frequently
✅ You want type-safe bindings
✅ You're wrapping complex Java APIs
